var BCLS = (function (window, document) {
  ...
  function startCSVStrings() {
    var i = 0,
      iMax;
    csvStr =
      '"ID","Name","Reference ID","Description","Date Added","Date Last Modified","State","Filename","Resolution","Duration(sec)","HLS Renditions (bitrate range KBPS)","MP4 Renditions (bitrate range KBPS)","FLV Renditions (bitrate range KBPS)","Total Rendition Size (MB)",\r\n';
  }
  ...
  function writeReport() {
    ...
    if (videosArray.length > 0) {
      ...
      for (i = 0; i < iMax; i += 1) {
        ...
        hlsLowRate =
          video.hlsRenditions.length > 0
            ? video.hlsRenditions[0].encoding_rate / 1000
            : 0;
        hlsHighRate =
          video.hlsRenditions.length > 0
            ? video.hlsRenditions[video.hlsRenditions.length - 1]
                .encoding_rate / 1000
            : 0;
        mp4LowRate =
          video.mp4Renditions.length > 0
            ? video.mp4Renditions[0].encoding_rate / 1000
            : 0;
        mp4HighRate =
          video.mp4Renditions.length > 0
            ? video.mp4Renditions[video.mp4Renditions.length - 1]
                .encoding_rate / 1000
            : 0;
        flvLowRate =
          video.flvRenditions.length > 0
            ? video.flvRenditions[0].encoding_rate / 1000
            : 0;
        flvHighRate =
          video.flvRenditions.length > 0
            ? video.flvRenditions[video.flvRenditions.length - 1]
                .encoding_rate / 1000
            : 0;
        ...
        csvStr +=
          ...
          video.duration / 1000 +
          '","' +
          video.hlsRenditions.length +
          " (" +
          hlsLowRate +
          "-" +
          hlsHighRate +
          ')","' +
          video.mp4Renditions.length +
          " (" +
          mp4LowRate +
          "-" +
          mp4HighRate +
          ')","' +
          video.flvRenditions.length +
          " (" +
          flvLowRate +
          "-" +
          flvHighRate +
